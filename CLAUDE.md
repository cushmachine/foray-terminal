...
On a box installed before the rename, the unit is still `nest-tmux.service` and sessions are on the machine's default tmux socket (plain `tmux ls`, `nest_<name>` prefix). `ecosystem.config.cjs` itself carries no `FORAY_TMUX_SOCKET` key — unset defaults to Foray's own `foray` socket — so a box like this one needs an **untracked `ecosystem.local.cjs`** next to it, exporting `{ env: { FORAY_TMUX_SOCKET: '' } }`; `ecosystem.config.cjs` merges that file's env over its own at pm2-start time (see the bottom of that file). That file is not shipped and nothing creates it for you: skip it and the next deploy switches this box to the `foray` socket, and the sessions still on the default one silently stop appearing in the sidebar (they are not lost — they stay in `nest-tmux.service` and can still be attached by hand — but the app can no longer see or reconnect to them). `ensure-tmux-unit.sh` leaves `nest-tmux.service` strictly alone either way. The same rule applies: never `systemctl stop` or `restart nest-tmux` either.
