...
Nothing else may build into the live `dist/`. The server stamps itself from git when it starts and the page is stamped at build time, so a `vite build` that lands in `dist/` without a restart leaves the server on one commit and the page it serves on another. That is what `"prepack": "npm run build"` did on every `npm pack` and `npm publish`. Packing now checks `dist/` instead of writing it (`scripts/prepack.ts`): it refuses to pack a bundle that is not this checkout's, and says to deploy first. If you do build by hand, deploy afterwards.
